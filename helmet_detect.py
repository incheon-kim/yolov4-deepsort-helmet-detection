...
def cvDrawBoxes(detections, img):
    ...
    for detection in detections:
        ...
        cv2.rectangle(img, pt1, pt2, tuple(colors[detection[0]]), 1)
        cv2.putText(img,
                    detection[0].decode() +
                    " [" + str(round(detection[1] * 100, 2)) + "]",
                    (pt1[0], pt1[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    colors[detection[0]], 2)
    return img
...
def YOLO(videopath):
    ...
    while True:
        ...
        if ret:
            ...
            # get inference information (class, probability, (x,y,width,height))
            detections = darknet.detect_image(netMain, metaMain, darknet_image, thresh=0.25)
            # resize bbox to match size of original image
            detections = resizeDetections(frame_rgb.shape, network_image_size, detections)
